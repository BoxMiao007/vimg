...
- `--layout 1` 是固定的 14 格一截：4 列、5 个行单位，左上和右下各一个 2×2 大格，正中间一行是 4 个小格。格与格之间留 8 像素黑缝，大格盖住内部那条缝，左右外缘留同样宽的一条，上下外缘不留。从第二截起，截与截之间多一行 4 个小格，把上下大格隔开。`vcs` 的 `-n` 在这个版式下是截数，默认 1。不写 `--layout` 仍是等大网格。
