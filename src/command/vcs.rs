...
///
/// `-l 1`（`--layout 1`）改用固定版式。一截 14 格，4 列、5 个行单位，左上和右下各一个 2×2 大格，正中间一行是 4 个小格。格与格之间留 8 像素黑缝，大格盖住内部那条缝；左右外缘各留 8 像素，上下外缘不留。这时 `-n` 是截数，默认 1。从第二截起，两截之间多一行 4 个小格，把上下两个大格隔开，所以 1 截 14 张，2 截 32 张，3 截 50 张。`-c`、`-H`、`-W` 被忽略并提示。小格高度固定 216，宽度按画面比例换算。
///
...
pub struct Vcs {
    /// 等大网格的列数。
    ///
    /// 必填，除非 `-l 1`。版式 1 固定 4 列，写了也忽略，并在终端提示。格数不能整除列数时，最后一行不满。
    ...
    ///
    /// 与 `-H` 取一，不能同时写。`-l 1` 时忽略并提示：小格高度固定 216，宽度按比例换算。
    #[arg(
        long,
        short = 'W',
        value_name = "像素",
    ...
    ///
    /// 与 `-W` 取一，不能同时写。等大网格必须写其中一个。`-l 1` 时忽略并提示。
    #[arg(
        long,
        short = 'H',
        value_name = "像素",
...
impl Vcs {
    pub fn run(mut self) -> anyhow::Result<()> {
        ...
        let header_band = self.header_band(&extract, &temp_dir, layout_small_w)?;
