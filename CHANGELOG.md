# Unreleased
* 默认参数栏改为九行：文件名、大小、时长、分辨率、视频码率与总码率、像素格式、视频解码器、音频解码器、流数量。
* 参数栏叠上网格后，总高度补成偶数，避免 YUV 4:2:0 编码器拒绝奇数高度。
* 参数栏默认字体改为内嵌 MiSans。`--font` 和 `VIMG_FONT` 仍可覆盖。
* 时间戳字号改为短边的 14%、最大 30 像素，版式 1 大小格一致；描边和阴影略收。
* `vcs` 与 `join` 的 `--layout` 增加简写 `-l`。
* 启动帮助列出每个子命令的全部选项和说明，不必再进一层才能看到。
...
* `vcs` 默认在网格上方绘制参数栏。`--no-info` 关闭，`--info-all` 追加其余轨、总帧数和容器。
* `join` 增加 `--video`。给出后绘制同一块参数栏；`--info-all` 必须同时给出 `--video`。
* `--font` 与环境变量 `VIMG_FONT` 指定参数栏字体，覆盖内嵌 MiSans。
